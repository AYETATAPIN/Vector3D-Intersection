...
void testVectorProduct() {
    ...
    assert(vector_product(first, second) == Vector3D(0, 0, 1));
    cout << "Vector product test passed\n";
}

void testScalarProduct() {
    const Vector3D first(1, 2, 3);
    const Vector3D second(4, 5, 6);
    assert(fabs(scalar_product(first, second) - 32) < INACCURACY);
    cout << "Scalar product test passed\n";
}
...
void testIntersection() {
    ...
    assert(Vector3D(0.5, 0.5, 0) == intersection);
    cout << "Intersection test passed\n";
}

void testCollinearSegments() {
    const Vector3D start1(0, 0, 0);
    ...
    try {
        ...
        assert(false && "Collinear segments test failed");
    } catch (const invalid_argument &e) {
        assert(e.what() == string("Segments are parallel and do not intersect"));
    }
    cout << "Collinear segments test passed\n";
}

void testSameSegments() {
    const Vector3D start1(0, 0, 0);
    const Vector3D end1(1, 0, 0);
    ...
    try {
        Intersect(segment1, segment2);
        assert(false && "Same segments test failed");
    } catch (const invalid_argument &e) {
        assert(
            e.what() ==
            string("Segments are the same and the intersection is an interval with start {0.000000, 0.000000, 0.000000} and end {1.000000, 0.000000, 0.000000}"));
    }
    cout << "Same segments test passed\n";
}

void testSameLineNoIntersectionSegments() {
    const Vector3D start1(0, 0, 0);
    ...
    try {
        Intersect(segment1, segment2);
        assert(false && "Same line segments test failed");
    } catch (const invalid_argument &e) {
        assert(e.what() == string("Segments are on the same line and do not intersect"));
    }
    cout << "Same line segments no intersection test passed\n";
}

void testSameLineIntersectionSegments() {
    const Vector3D start1(0, 0, 0);
    const Vector3D end1(1, 1, 1);
    ...
    try {
        ...
        assert(
            e.what() ==
            string("Segments intersection is an interval with start {0.500000, 0.500000, 0.500000} and end {1.000000, 1.000000, 1.000000}"));
    }
    cout << "Same line segments intersection test passed\n";
}

void testNoIntersectionSegments() {
    const Vector3D start1(0, 0, 0);
    const Vector3D end1(1, 1, 1);
    ...
    try {
        ...
        assert(e.what() == string("Segments do not intersect"));
    }
    cout << "No intersection segments test passed\n";
}
...
int main(int argc, char *argv[]) {
    ...
    testSameSegments();
    testSameLineNoIntersectionSegments();
    testSameLineIntersectionSegments();
    testNoIntersectionSegments();
